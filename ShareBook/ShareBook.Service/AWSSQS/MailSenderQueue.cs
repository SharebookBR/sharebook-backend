...
public class MailSenderQueue : IAwsSqsQueue<MailSenderMessage>
{
    private readonly AwsSqsSettings _AwsSqsSettings;
    private readonly AmazonSQSClient _amazonSQSClient;

    public MailSenderQueue(IOptions<AwsSqsSettings> AWSSQSSettings)
    {
    ...
    public async Task SendMessage(MailSenderMessage message)
    {
        if (_AwsSqsSettings.IsActive)
        {
    ...
    public async Task<MailSenderMessage> GetMessage()
    {
        if (!_AwsSqsSettings.IsActive)
        {
        ...
        if (result.Messages.Count > 0)
        {
            ...
            var firstMessage = System.Text.Json.JsonSerializer.Deserialize<MailSenderMessage>(firstMessageTemp);
