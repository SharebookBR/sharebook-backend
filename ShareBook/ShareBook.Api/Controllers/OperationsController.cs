...
public class OperationsController : Controller
{
    ...
    [Authorize("Bearer")]
    [AuthorizationFilter(Permissions.Permission.ApproveBook)] // adm
    public async Task<IActionResult> ImporterItemsAsync([FromQuery] int? sourceId, [FromQuery] string status, [FromQuery] int? position, [FromQuery] string sort, [FromQuery] int page = 1, [FromQuery] int pageSize = 50, CancellationToken cancellationToken = default)
    {
        var items = await _importerDashboardService.GetItemsAsync(sourceId, status, position, sort, page, pageSize, cancellationToken);
        return Ok(items);
    }
