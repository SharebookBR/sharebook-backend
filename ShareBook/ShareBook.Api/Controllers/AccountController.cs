...
public class AccountController : ControllerBase 
{
    ...
    [Throttle(Name = "unsubscribe", Seconds = 5, VaryByIp = false)]
    [HttpGet("unsubscribe")]
    public async Task<IActionResult> unsubscribe([FromQuery] string unsubToken)
    {
        try
        {
            await _userService.Unsubscribe(unsubToken);
            return Ok("Você se desinscreveu com sucesso.");
        }
        catch (Exception ex)
        {
